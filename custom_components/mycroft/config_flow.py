    ...
    async def async_step_user(
        ...
        try:
            ...
            return self.async_create_entry(
                title="Mycroft Conversation", data=user_input
            )
...
class OptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_init(
        ...
        schema = mycroft_config_option_schema(self.config_entry.options)
        return self.async_show_form(
            step_id="init",
...
def mycroft_config_option_schema(options: MappingProxyType[str, Any]) -> dict:
    """Return a schema for OpenAI completion options."""
    if not options:
        options = DEFAULT_OPTIONS
    return {
        vol.Required(CONF_PROMPT, default=options.get(CONF_PROMPT)): TemplateSelector(),
        vol.Required(HOME_INFO_TEMPLATE, default=options.get(HOME_INFO_TEMPLATE)): TemplateSelector(),
